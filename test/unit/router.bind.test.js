...
describe('Router.bind', function() {
  ...
  $Router.bind('/bar', RESPOND.HELLO)
    ...
    .test(function() {
      // Express 4 (correctly) attempts to send an 'Allow' header via
      // `res.set('Allow', 'GET, POST')`. This isn't supported by the
      // mock-res library being used for testing here, so let's skip this
      // test.
      it.skip('should send expected response (options /bar)', function(done) {
        this.sails.request({
          url:    '/bar',
          method: 'options'
        }, function(err, res, body){
